 ...
 * Tema: Concurrencia
 * Objetivo: Hacer una metodologÃ­a para la implementaciÃ³n de la multiplicaciÃ³n de matrices.
 * La idea principal, es construir paso a paso la implementaciÃ³n para hacer uso de la
 * biblioteca PTHREAD. Se implementa el Algoritmo ClÃ¡sico de multiplicaciÃ³n de matrices, para
 * matrices cuadradas, es decir, la dimensiÃ³n de filas es igual a la de columnas.
 * Inicialmente se genera el programa o codigo de forma general.
 * Posteriromente, 
 * A continuaciÃ³n se describen los paso
 *  - Reserva de memoria
 *      - CreaciÃ³n de punteros para matrices del tipo doble
 *      - AsignaciÃ³n de memoria
 *      - Ingreso de argumentos de entrada (DimensiÃ³n matriz, nÃºmero de hilos)
 *      - ValidaciÃ³n argumentos de entrada
 *      - Inicializar las matrices
 *      - Imprimir las matrices
 *      - FunciÃ³n para inicializar las matrices
 *      - FunciÃ³n para imprimir las matrices
 *      - Algoritmo clÃ¡sico de multiplicaciÃ³n matrices
 *  - Se verifica el resultado
 *      - FunciÃ³n para multiplicaciÃ³n las matrices
 *      - DeclaraciÃ³n vector de hilos
 *      - CreaciÃ³n de hilos segÃºn tamaÃ±o de vector de hilos
 *      - Crear estructura de datos que encapsule los argumentos de entrada de la funciÃ³n MM
 * ImplementaciÃ³n de paralelismo: resoluciÃ³n de la multiplicaciÃ³n de matrices
 *  - Se crea el vector de hilos
 *  - Se tiene pendiente la exclusiÃ³n de los hilos
 *  - Se pasa a globales las matrices
 *  - Encapsular los datos para enviarlos a la funciÃ³n MxM
 *  - Se desencapsulan los datos dentro de la funciÃ³n MxM (descomprimen)
*************************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
//funcion para la impresion de matrices, mientras que la dim sea menor a 10
void impMatriz(int N, double *m) {
    if (N <= 10) {
        printf("\n");
        for (int i = 0; i < N * N; i++) {
...
void multMM(int N, const double *A, const double *B, double *C) {
    for (int i = 0; i < N; i++) { //recorre fila por fila de matriz A
        for (int j = 0; j < N; j++) { ///recorre columna por columna de matriz B
            ...
            C[i * N + j] = suma;//se guarda el valor calculado en la posición (i,j) de C
        }
    }
}
